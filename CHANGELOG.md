...
- **Truthful daily dogfood status:** failed or incomplete required checks fail the workflow after logs and reports are collected. A bounded provider preflight identifies missing credentials and exhausted quota without presenting them as agent regressions; the rolling issue closes only after a complete successful run.
