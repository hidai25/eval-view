"""Source code for the demo FastAPI agent that `evalview init` writes.

The full agent.py file is held here as a constant so init_cmd.py stays
focused on the init flow itself. Edits here change the agent that
`init` materializes into demo-agent/agent.py.
"""
...
_DEMO_AGENT_REQUIREMENTS = "fastapi>=0.100.0\nuvicorn>=0.23.0\npydantic>=2.0.0\n"
...
def _create_demo_agent(base_path: Path) -> None:
    ...
    (demo_agent_dir / "agent.py").write_text(DEMO_AGENT_SOURCE)
    (demo_agent_dir / "requirements.txt").write_text(_DEMO_AGENT_REQUIREMENTS)
