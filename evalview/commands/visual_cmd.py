...
@click.pass_context
def inspect_cmd(ctx: click.Context, target: str, title: str, notes: str, no_open: bool, output: Optional[str]) -> None:
    """Deprecated: use `evalview visualize` instead."""
    console.print(
        "[yellow]warning:[/yellow] `inspect` is deprecated; "
        "use `evalview visualize` instead.",
    )
    ctx.invoke(
        visualize_cmd,
        target=target,
