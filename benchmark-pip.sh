#!/usr/bin/env bash
#
# benchmark-pip.sh - Benchmark pip package manager performance for EvalView
#
# Usage: ./benchmark-pip.sh [iterations]
#   iterations: Number of benchmark iterations (default: 3)
#
# Output: benchmark-pip-results.json
#
...
readonly RESULTS_FILE="${PROJECT_DIR}/benchmark-pip-results.json"
...
run_single_iteration() {
    ...
    # Activate venv - use explicit path to python instead of source
    local VENV_PYTHON="${VENV_DIR}/bin/python"
    local VENV_PIP="${VENV_DIR}/bin/pip"

    # Upgrade pip (NOT timed, per user request)
    log_info "Upgrading pip (not timed)..."
    ...
    "${VENV_DIR}/bin/pytest" tests/ -v --tb=short > /dev/null 2>&1 || true
    pytest_end=$(get_time)
    ...
    "${VENV_DIR}/bin/black" evalview/ tests/ --line-length 100 --check > /dev/null 2>&1 || true
    black_end=$(get_time)
    ...
    ruff_start=$(get_time)
    "${VENV_DIR}/bin/ruff" check evalview/ tests/ > /dev/null 2>&1 || true
    ruff_end=$(get_time)
    ...
    mypy_start=$(get_time)
    "${VENV_DIR}/bin/mypy" evalview/ --strict > /dev/null 2>&1 || true
    mypy_end=$(get_time)
...
main() {
    ...
    log_info "=========================================="
    log_info "PIP BENCHMARK FOR EVALVIEW"
    ...
    # Get system info
    local timestamp python_version pip_version os_name os_version
    timestamp=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
    python_version=$(python3 --version 2>&1 | awk '{print $2}')
    pip_version="${VENV_DIR}/bin/pip --version 2>/dev/null | awk '{print \$2}'" || echo "unknown"
    # Get pip version from last venv (may not exist after cleanup, use system)
